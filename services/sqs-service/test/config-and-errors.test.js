...
test('config displayName never contains the service name', () => {
  for (const item of awsConfigItems) {
    assert.ok(!/sqs/i.test(item.displayName), `${item.name} displayName leaks service name`)
  }
})
...
test('createLogger prefixes messages and exposes 4 levels', () => {
  ...
  assert.ok(lines[0].startsWith('[SQS Service] info:'))
  assert.ok(lines[1].startsWith('[SQS Service] error:'))
})
